...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 text-center"><p class="text-3xl font-bold text-amber-500"><x-heroicon-s-star class="w-5 h-5 inline-block text-yellow-400 mr-1"/> {{ $avgRating }}</p><p class="text-xs text-gray-500 mt-1">Rating Rata-rata</p></div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        @for($i = 5; $i >= 1; $i--)
            <div class="flex items-center gap-2 text-xs mb-1"><span class="w-3">{{ $i }}<x-heroicon-s-star class="w-3 h-3 inline-block text-yellow-400 align-text-top mr-1"/></span><div class="flex-1 bg-gray-100 rounded-full h-2"><div class="bg-amber-400 rounded-full h-2" style="width: {{ $reviewCount > 0 ? ($ratingDist[$i] / $reviewCount * 100) : 0 }}%"></div></div><span class="w-6 text-right text-gray-500">{{ $ratingDist[$i] }}</span></div>
        @endfor
    </div>
</div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 {{ $review->is_hidden ? 'opacity-60' : '' }}">
        ...
        @else
            <div x-data="{ open: false }" class="mt-4">
                <button @click="open = !open" class="text-sm text-ocean-600 hover:text-ocean-700 font-semibold"><x-heroicon-o-chat-bubble-left-ellipsis class="w-5 h-5 inline-block mr-1.5 -mt-1 text-current"/> Balas ulasan</button>
